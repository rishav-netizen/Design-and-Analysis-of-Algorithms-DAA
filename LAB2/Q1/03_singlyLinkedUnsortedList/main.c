...
int main(void)
{
    ...
    Input(&D, count);
    printf("\nUnsorted singly linked dictionary:\n");
    Display(&D);
...
/* Adds at the head. Enter distinct keys to preserve dictionary semantics. */
bool Insert(Dict *D, int key)
{
...
/* A predecessor link must be found before deleting from a singly linked list. */
bool Delete(Dict *D, Node *target)
{
    Node *previous = NULL;
    Node *current = D->head;
